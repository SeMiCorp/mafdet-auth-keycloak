...
resource "keycloak_realm" "mafdet" {
  lifecycle {
  ...
  security_defenses {
    headers {
      ...
      content_security_policy             = "frame-src 'self'; frame-ancestors 'self' %{if var.mafdet_url != ""}${var.mafdet_url}%{else}*%{endif}; object-src 'none';"
...
module "test_users" {
  ...
  mafdet_realm = keycloak_realm.mafdet.id
}
